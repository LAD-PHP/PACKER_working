...
variable "iso_url" {
  type    = string
  default = "D:/VM/PACKER/CentOS-7-x86_64-NetInstall-2009.iso"
}
...
source "vmware-iso" "autogenerated_2" {
  ...
  headless         = false
  http_directory   = "D:/VM/PACKER/http"
  iso_checksum     = "${var.iso_checksum}"
  iso_url          = "${var.iso_url}"
  shutdown_command = "echo 'packer'|sudo -S /sbin/halt -h -p"
  ssh_password     = "${var.ssh_password}"
...
build {
  ...
  provisioner "ansible-local" {
    playbook_file = "D:/VM/PACKER/scripts/setup.yml"
  }

  provisioner "shell" {
    execute_command = "echo 'packer'|{{ .Vars }} sudo -S -E bash '{{ .Path }}'"
    scripts         = ["D:/VM/PACKER/scripts/cleanup.sh"]
  }
